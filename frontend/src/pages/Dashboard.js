...
const Dashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [assignmentsRes, coursesRes, notificationsRes, liveRes] = await Promise.all([
          API.get('/api/courses/student/assignments'),
          API.get('/api/courses'),
          API.get('/api/courses/student/notifications'),
          API.get('/api/live-sessions') // New endpoint
        ]);
        ...
        <AnimatePresence>
          {liveSessions.length > 0 && (
            <motion.div
              ...
              className="mb-8"
            >
              <div className="bg-gradient-to-r from-red-600 to-rose-600 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden group">
                ...
                <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center space-x-5">
                    <div className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center animate-pulse border border-white/30 shadow-inner">
                    ...
                    <div>
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <h2 className="text-sm font-bold uppercase tracking-widest text-white/90">Ongoing Live Classes</h2>
                      </div>
                      <h3 className="text-2xl font-extrabold tracking-tight">
                        {liveSessions[0].teacherName}'s Live Lab Session
                      </h3>
                      <p className="text-red-100 text-sm mt-1 font-medium italic">
                        Join now to participate in real-time experiments and live discussion!
                      </p>
                    </div>
                  </div>

                  <Link
                    to={`/live-session/${liveSessions[0].courseId}`}
                    className="inline-flex items-center justify-center px-8 py-4 bg-white text-red-600 rounded-xl font-bold shadow-2xl hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all duration-300 group"
                  >
