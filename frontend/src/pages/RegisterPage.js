...
const RegisterPage = () => {
  ...
  useEffect(() => {
    const fetchInstitutions = async () => {
      ...
      try {
        ...
        // Filter only verified institutions for registration
        const verified = (res.data.institutions || []).filter(inst => inst.status === 'Active');
  ...
  const validateForm = () => {
    ...
    if ((formData.role === 'student' || formData.role === 'teacher') && !formData.institutionId) {
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              Join Virtual Lab LMS and start your learning journey
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              >
                ...
                <option value="education_officer">Education Officer (Govt.)</option>
              </select>
            </div>

            {/* Institution Selection (only for students and teachers) */}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={handleChange}
                    className={`input ${errors.institutionId ? 'input-error' : ''}`}
                    disabled={fetchingInstitutions}
                  >
                ...
                {errors.institutionId && (
                  <p className="mt-1 text-sm text-error-600">{errors.institutionId}</p>
                )}
                {institutions.length === 0 && !fetchingInstitutions && (
                  <p className="mt-1 text-xs text-amber-600 font-medium">
                    Don't see your school? It might be pending verification by an Education Officer.
                  </p>
                )}
              </div>
            )}
                ...
                <div className="mt-2 space-y-1">
                  {passwordRequirements.map((req, index) => (
                    <div key={index} className="flex items-center space-x-2 text-xs">
                      <CheckCircle 
                        className={`w-3 h-3 ${req.met ? 'text-success-500' : 'text-gray-300'}`} 
                      />
