...
export const autoGradeTemplates = {
  ...
  arduino: {
    name: "Arduino Uno Robotics Lab",
    description: "Auto-grades C++ code logic and circuit behavior observations",
    criteria: {
      rules: {
        totalPoints: 50,
        expectedValues: {
          pin13: { value: 'OUTPUT', tolerance: 0, points: 10 },
          delayMs: { value: 1000, tolerance: 100, points: 10 },
          serialRate: { value: 9600, tolerance: 0, points: 10 }
        }
      },
      rubric: {
        totalPoints: 50,
        sections: {
          'Code': {
            indicators: ['void setup', 'void loop', 'pinMode', 'digitalWrite'],
            points: 15,
            minLength: 50
          },
          'Observations': {
            indicators: ['blink', 'LED', 'frequency', 'noticed'],
            points: 15,
            minLength: 50
          },
          'Analysis': {
            indicators: ['embedded', 'logic', 'circuit', 'GPIO'],
            points: 10,
            minLength: 40
          }
        },
        keywords: {
          list: ['setup', 'loop', 'pinMode', 'digitalWrite', 'delay', 'serial', 'voltage'],
          pointsPerKeyword: 2,
          maxPoints: 10
        },
        minLength: 200,
  ...
  pendulum: {
    name: "Pendulum Lab",
    description: "Auto-grades pendulum period calculations and gravity measurements",
    criteria: {
      rules: {
        totalPoints: 50,
        expectedValues: {
          period: { value: 2.01, tolerance: 0.05, points: 25 },
          gravity: { value: 9.8, tolerance: 0.1, points: 25 }
        }
      },
      rubric: {
        totalPoints: 50,
        sections: {
          'Results': {
            indicators: ['result', 'period', 'length', 'time'],
            points: 10,
            minLength: 30
          },
          'Observations': {
            indicators: ['observe', 'noticed', 'found', 'swing'],
            points: 15,
            minLength: 50
          },
          'Analysis': {
            indicators: ['analysis', 'harmonic', 'conclude', 'L/g'],
            points: 15,
            minLength: 50
          }
        },
        keywords: {
          list: ['harmonic', 'oscillation', 'period', 'frequency', 'gravity', 'length', 'mass'],
          pointsPerKeyword: 2,
          maxPoints: 10
        },
        minLength: 200,
