...
const Navbar = () => {
  ...
  const isOfficer = user?.role === 'education_officer';
  const navItems = isOfficer
    ? [
      { path: '/transparency', label: 'Dashboard', icon: Globe },
      { path: '/courses', label: 'Courses', icon: BookOpen },
      { path: '/resources', label: 'Resources', icon: FileText },
      { path: '/manage-schools', label: 'Schools', icon: School },
    ]
    : isTeacher
    ? [
      ...
      { path: '/courses', label: 'Courses', icon: BookOpen },
      { path: '/resources', label: 'Resources', icon: FileText },
      { path: '/live-session', label: 'Live Session', icon: Video },
    ]
    : [
    ...
    <nav className="bg-white shadow-soft border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive(item.path)
          ...
          {/* Lite Mode Toggle + User Menu */}
          <div className="flex items-center space-x-3">
            {/* Lite Mode Toggle */}
            <button
              onClick={toggleLiteMode}
              className={`flex items-center space-x-1 px-3 py-1.5 rounded-full text-xs font-medium transition-colors border ${
                isLiteMode
                  ? 'bg-yellow-100 text-yellow-700 border-yellow-300'
                  : 'bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200'
              }`}
              title={isLiteMode ? 'Switch to Full Mode' : 'Switch to Lite Mode (Low Bandwidth)'}
            >
          ...
          {isAuthenticated ? (
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link
                to="/register-school"
                className="text-sm font-medium text-gray-600 hover:text-primary-600 transition-colors hidden lg:block"
              >
                Register School
              </Link>
              <Link
