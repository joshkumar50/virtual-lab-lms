    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="p-6">
          ...
          {/* AI Grading Insights */}
          {submission.gradeInfo && submission.gradeInfo.autoGraded && (
            <div className="mb-6 bg-indigo-50 border border-indigo-100 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-indigo-900 flex items-center">
                  <span className="mr-2">🤖</span> AI Grading Insights
                </h3>
                <span className="text-xs font-medium bg-indigo-200 text-indigo-800 px-2 py-1 rounded-full">
                  Model: {submission.gradeInfo.aiModel || 'Ollama'}
                </span>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                {submission.gradeInfo.breakdown && Object.entries(submission.gradeInfo.breakdown).map(([key, value]) => (
                  <div key={key} className="bg-white p-2 rounded border border-indigo-100">
                    <span className="block text-xs text-indigo-500 capitalize">{key.replace(/_/g, ' ')}</span>
                    <span className="block font-bold text-indigo-900">{value} pts</span>
                  </div>
                ))}
              </div>

              {submission.gradeInfo.improvementSuggestion && (
                <div className="mb-3">
                  <h4 className="text-xs font-bold text-indigo-800 uppercase tracking-wide mb-1">Improvement Suggestion</h4>
                  <p className="text-sm text-indigo-800 italic">"{submission.gradeInfo.improvementSuggestion}"</p>
                </div>
              )}

              <div className="text-xs text-indigo-600">
                <p>Teacher Note: You can override the AI grade below.</p>
              </div>
            </div>
          )}
