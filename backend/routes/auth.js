...
router.post('/register', [
  ...
  body('role')
    .isIn(['student', 'teacher', 'education_officer'])
    .withMessage('Role must be student, teacher, or education_officer')
...
router.post('/login', [
  ...
  if (!isPasswordValid) {
    console.log(`Debug: Login failed for ${email} - Invalid password`);
    return res.status(401).json({
      success: false,
      message: 'Invalid email or password'
    });
