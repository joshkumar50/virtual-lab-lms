...
const { autoGradeWithAI } = require('../utils/autoGrader');
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // If user is logged in and has an institutionId (e.g., student), only show courses from that institution
...
router.get('/student/assignments', authMiddleware, async (req, res) => {
  try {
    ...
    // Base query: find courses either published or where the student is specifically enrolled
    let courseQuery = {
      $or: [
        { status: 'published' },
    ...
    // If student has an institution, strictly silo the 'published' results to that institution
    if (req.user.institutionId) {
      courseQuery = {
        $or: [
          { status: 'published', institutionId: req.user.institutionId },
          { students: req.user._id }
        ]
      };
    }

    const courses = await Course.find(courseQuery).populate('createdBy', 'name email');
...
router.post('/:courseId/assignments', authMiddleware, async (req, res) => {
  try {
    ...
    const { title, description, dueDate, assignedStudents, autoGrade, gradingCriteria, maxScore, labType, labTitle } = req.body;
    ...
    const assignment = {
      ...
      createdAt: new Date(),
      // Store the lab type and title so students are routed to the correct lab
      labType: labType || 'electronics',
...
router.put('/:courseId/assignments/:assignmentId', authMiddleware, async (req, res) => {
  try {
    ...
    const { title, description, dueDate, status, labType, labTitle } = req.body;
    if (title) assignment.title = title;
    if (description) assignment.description = description;
    if (dueDate) assignment.dueDate = dueDate;
    if (status) assignment.status = status;
    if (labType) assignment.labType = labType;
    if (labTitle) assignment.labTitle = labTitle;
  ...
  try {
    ...
    if (assignment.dueDate && new Date() > new Date(assignment.dueDate)) {
      return res.status(400).json({
        message: 'Deadline has passed. Submissions are no longer accepted for this assignment.',
    ...
    // Check previous submissions by this student
    const previousSubmissions = assignment.submissions.filter(s =>
      s.student && (s.student.equals ? s.student.equals(req.user._id) : String(s.student) === String(req.user._id))
    );

    const maxAttempts = assignment.maxAttempts || 3;
    if (previousSubmissions.length >= maxAttempts) {
      return res.status(400).json({
        message: `Maximum attempts (${maxAttempts}) reached. You cannot resubmit.`
      });
    }
    ...
    if (assignment.autoGrade) {
      console.log(`🤖 AI Auto-grading assignment "${assignment.title}" (Attempt ${attemptNumber})...`);

      // Determine subject from assignment title or course title (fallback)
      const subject = assignment.title || course.title || "General Science";

      try {
        gradeResult = await autoGradeWithAI(content, subject);

        if (gradeResult.error || !gradeResult.autoGraded) {
          console.log(`⚠️ AI Grading unavailable/failed: ${gradeResult.feedback ? gradeResult.feedback[0] : 'Unknown error'}`);
          submission.status = 'pending_manual_review';
          submission.grade = {
            marks: 0,
            feedback: "AI Grading Pending Manual Review. " + (gradeResult.feedback ? gradeResult.feedback.join(' ') : ""),
          ...
          submission.grade = {
            ...
            // Store structured feedback for UI
            feedbackArray: gradeResult.feedback,
            detailedFeedback: gradeResult.detailedFeedback,
            improvementSuggestion: gradeResult.improvementSuggestion, // Save improvement suggestion
            breakdown: gradeResult.breakdown,
            gradedBy: null, // System graded
            gradedAt: new Date(),
            autoGraded: true,
            wasAutoGraded: true,
            aiModel: gradeResult.aiModel
          };
          console.log(`✅ AI Graded: ${gradeResult.score}/100`);
        }
      } catch (error) {
        console.error("Critical AI Grading Error:", error);
        submission.status = 'pending_manual_review';
        submission.grade = {
          marks: 0,
          feedback: "System Error during grading. Needs manual review.",
          autoGraded: false
        };
      }
    }
